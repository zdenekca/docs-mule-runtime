...
TESTING, NEW CONTENT TO REMOVE LATER
...
You can deploy Mule applications to MuleSoft's cloud platform via xref:runtime-manager::cloudhub.adoc[CloudHub] and manage them via xref:runtime-manager::index.adoc[Runtime Manager]. Runtime Manager can be used to manage local deployments as well, making it more versatile than its predecessor xref:3.8@mule-management-console::index.adoc[Mule Management Console], which is used only for local Mule deployments.
