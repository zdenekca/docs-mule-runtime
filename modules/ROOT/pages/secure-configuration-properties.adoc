...
Since version 1.1.0 of the secure configuration properties module, you can encrypt the entire file (not only the values of the properties). To use a file that its encrypted you should add the `fileLevelEncryption` attribute, and set it to `true`: 
...
Note that Secure Configuration Properties can depend on other Secure Configuration Properties, but not on configuration properties defined by the xref:configuring-properties.adoc[Configuration Properties Component].
...
In the case of using `file-level` mode, the output is a file entirely encrypted. For the same example above, the output will be: 
----
